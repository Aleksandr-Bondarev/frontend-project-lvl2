const bracketIndent = (depthValue, indent = ' ', extraIndentCounter = 0) =>
  indent.repeat(4 * depthValue + extraIndentCounter);

const indentMaker = (depthValue, indent = ' ') => {
  return bracketIndent(depthValue, indent, 2);
};
...
const stringify = (value, depth) => {
  if (typeof value === 'object' && value !== null) {
    const lines = Object.entries(value).map(
      ([key, val]) =>
        `${indentMaker(depth)}  ${key}: ${stringify(val, depth + 1)}`
    );
